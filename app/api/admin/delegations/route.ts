...
import { safeDecryptStaffField, maskName, maskPhone } from "@/lib/staff-crypto";
...
export async function GET() {
  try {
    ...
    if (error) {
      console.error("[delegations] list_active_delegations error", error);
    ...
    const staffById = Object.fromEntries(
      (staff ?? []).map((s) => [
        s.id,
        {
          name: maskName(safeDecryptStaffField(s.name_enc)),
          phone: maskPhone(safeDecryptStaffField(s.phone_enc)),
        },
      ])
    );
    ...
    console.error("[delegations] unexpected error", err);
    return NextResponse.json({ error: "위임 현황을 불러오지 못했습니다." }, { status: 500 });
  }
}
