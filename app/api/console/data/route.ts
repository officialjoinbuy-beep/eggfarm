...
export async function GET() {
  ...
  const [{ data: limits }, { data: requests }, { data: history }, usersRes] = await Promise.all([
    supabase
      .from("account_limits")
      .select(
        "owner_id, owner_phone, campaign_limit, campaigns_created_count, trial_exhausted_at, created_at, last_purchase_product_name, last_purchase_at"
      )
  ...
  const phoneByOwnerId: Record<string, string | null> = {};
  for (const l of limits ?? []) {
    phoneByOwnerId[l.owner_id] = (l as { owner_phone?: string | null }).owner_phone ?? null;
  }

  const signups = (limits ?? []).map((l) => ({
    ...l,
    email: emailByOwnerId[l.owner_id] ?? "(알 수 없음)",
    phone: phoneByOwnerId[l.owner_id] ?? null,
  }));
  const requestsWithEmail = (requests ?? []).map((r) => ({
    ...r,
    email: emailByOwnerId[r.owner_id] ?? "(알 수 없음)",
    phone: phoneByOwnerId[r.owner_id] ?? null,
  }));
  const historyWithEmail = (history ?? []).map((h) => ({
    ...h,
    email: emailByOwnerId[h.owner_id] ?? "(알 수 없음)",
    phone: phoneByOwnerId[h.owner_id] ?? null,
  }));
